...
#
# The legacy indexed-format helpers `_condition_to_lua` and `_action_to_lua`
# lived here until v1.19.66 — they rendered each cond/action as a Lua-source
# string for the pre-v0.9.50 `trig.actions = {[N] = "lua source"}` table.
# Since v0.9.50 every trigger goes through `_render_inline_condition` /
# `_render_inline_action` below (inline-dict format), so the helpers had
# zero callers for a year. Deleted in v1.19.66 dead-code sweep.
