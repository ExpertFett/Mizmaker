...
--[[
================================================================================
  AEGIS IADS v0.9.1-beta-networked -- Event-Driven IADS w/ runtime spawn
                                       discovery + networked dynamic sectors

  *** DYNAMIC VARIANT — DERIVED FROM aegis-iads-v0.9.0-beta-dynamic.lua ***
  *** UNTESTED IN DCS as of branch creation. Validate in a live mission   ***
  *** before relying on it. The CORE v0.8.4 file is unchanged and remains  ***
  *** the vetted/stable build.                                              ***

  WHAT'S DIFFERENT FROM v0.9.0
  ----------------------------
  v0.9.0 puts every dynamic SAM into sector "_AUTO" and runs it on autonomous
  EMCON only — even when a dynamic EWR is also up. v0.9.1 wires them together:

    * When a dynamic EWR is registered, the script scans every existing
      "_AUTO" SAM. Any SAM within `dynamicEwRangeKm` (default 80 km) of the
      new EW is PROMOTED into a fresh sector named "_DYN_<n>", its autonomous
      EMCON loop is cancelled, and it joins the regular EW-fed poll instead.
    * When a dynamic SAM spawns NEXT to an existing dynamic EW, it goes
      straight into that EW's "_DYN_<n>" sector — never touches _AUTO.
    * SAMs that are out of range of any dynamic EW stay in "_AUTO" with the
      v0.9.0 behaviour. Sectors with no live EW degrade gracefully back to
      autonomous EMCON if the EW dies (handled via _SectorHasEW fallback in
      the existing poll loop).
    * `_PollNextSector` lazily rebuilds `sectorPollOrder` once per full
      rotation so newly-promoted sectors join without restarting the timer
      (the doubled-poll concern called out in v0.9.0 — addressed by rebuild,
      not restart).

  WHAT'S DIFFERENT FROM v0.8.4 (carry-over from v0.9.0)
  -----------------------------------------------------
  v0.8.4 discovers air-defense groups ONCE at Activate() by parsing ME group
  NAMES (SAM-TYPE-SECTOR, EW-SECTOR, ...). That cannot work for units spawned
  at runtime by tools that can't set AEGIS-format group names (e.g. DCS Olympus
  / the DCS:OPT Live "Draw tool" IADS spawner) — Olympus auto-names groups.

  This variant adds TYPE-BASED dynamic discovery:
    * A S_EVENT_BIRTH ground handler watches for friendly (own-coalition)
      ground groups that spawn AFTER mission start.
    * Each new group is classified by UNIT TYPE (not name) against
      AEGIS.TYPE_TO_SYSTEM (track radars + SHORAD/PD launchers) and
      AEGIS.EWR_TYPES.

  C2 NOTE (the original v0.9.1-blocker that turned out to be a non-issue):
    `_SectorHasC2(sector)` returns true when `#sector.cmd == 0` — i.e. a
    sector with no CMD post passes the C2 gate by default. So a networked
    dynamic sector with EW+SAM but no CMD post still operates normally; it
    just can't be killed by losing a CMD post (since there isn't one to lose).
    That matches reasonable expectation: Olympus-spawned air defense has no
    organic command structure, so we don't pretend it does.

  CONFIG (in addition to all v0.8.4 options):
    dynamicDiscovery         = true  -- master switch (default ON in this file)
    dynamicDiscoveryDebounce = 3     -- s to wait after first unit birth so the
                                     --   whole group populates before classify
    dynamicStartupGrace      = 8     -- s after Activate() to IGNORE births, so
                                     --   pre-placed ME units aren't auto-adopted
    dynamicEwRangeKm         = 80    -- v0.9.1 networking radius — distance
                                     --   between a dynamic EW and a dynamic
                                     --   SAM for them to join the same
                                     --   sector. Set 0 to disable networking
                                     --   (back to pure v0.9.0 autonomous).
  Extend AEGIS.TYPE_TO_SYSTEM / AEGIS.EWR_TYPES below to support mod systems.

  --------------------------------------------------------------------------
  Original v0.8.4 phase notes:
  ...
  Version: 0.9.0-beta-dynamic  (derived from 0.8.4-beta; dynamic discovery added)
================================================================================
--]]
...
--
-- v0.8.4 keys off ME GROUP NAMES. Runtime spawns (Olympus / Live Draw tool)
-- can't set AEGIS-format names, so the dynamic handler classifies a spawned
-- group by the UNIT TYPES it contains. Map the *identifying* unit type of
-- each system (its tracking radar, or for SHORAD/PD the self-contained
-- launcher vehicle) to a SYSTEM_DB code. Detection is exact-match first,
-- then case-insensitive substring fallback (see AEGIS._ClassifyType).
--
-- The codes on the right MUST exist in AEGIS.SYSTEM_DB above. To support a
-- mod system, add its DCS unit type name here and (if new) a SYSTEM_DB entry.
---------------------------------------------------------------------------

AEGIS.TYPE_TO_SYSTEM = {
  -- Area SAM tracking / engagement radars (DCS + editor-vetted type names)
  ["SNR_75V"]                     = "SA2",     -- SA-2 Fan Song
  ["snr s-125 tr"]                = "SA3",     -- SA-3 Low Blow
  ["p-19 s-125 sr"]               = "SA3",     -- SA-3 Flat Face SR (site marker)
  ["RPC_5N62V"]                   = "SA5",     -- SA-5 Square Pair
  ["Kub 1S91 str"]                = "SA6",     -- SA-6 Straight Flush
  ["Hawk tr"]                     = "HAWK",    -- MIM-23 Hawk track radar
  ["Hawk sr"]                     = "HAWK",    -- Hawk search radar
  ["Patriot str"]                 = "PATRIOT", -- Patriot track radar
  ["Patriot cp"]                  = "PATRIOT", -- Patriot command post
  ["NASAMS_Radar_MPQ64F1"]        = "NASAMS",
  ["NASAMS_Command_Post"]         = "NASAMS",
  -- S-300 family (PS / PMU / V / VM / V4) track + search radars
  ["S-300PS 40B6M tr"]            = "SA10",
  ["S-300PS 40B6MD sr"]           = "SA10B",
  ["S-300PS 30N6 TRAILER tr"]     = "SA10B",
  ["S-300PMU1 40B6M tr"]          = "SA10C",
  ["S-300PMU1 30N6E tr"]          = "SA20A",
  ["S-300PMU2 92H6E tr"]          = "SA20A",
  ["S-300PMU2 30N6E2 mast tr"]    = "SA20B",
  ["S-400 92N6E mast tr"]         = "SA21",
  ["S-400 92H6E tr"]              = "SA20B",
  ["SAM SA-21 tr"]                = "SA21",
  ["S-300V 9S32 tr"]              = "SA12",
  ["S-300V 9S32 TR"]              = "SA12",
  ["S-300VM 9S32ME tr"]           = "SA12G",
  ["S-300VM 9S457ME sr"]          = "SA23",
  ["S-300VM 9S15M2 sr"]           = "SA23G",
  ["S-300V4 9S32M-1E tr"]         = "SA23V4",
  ["S-300V4 9S32ME tr"]           = "SA23V4",
  ["S-300V4 9S457ME sr"]          = "SA23V4G",
  -- Buk family
  ["SA-11 Buk SR 9S18M1"]         = "SA11",    -- Buk Snow Drift search radar
  ["SA-11 Buk LN 9A310M1"]        = "SA11",    -- Buk TELAR (self-tracking)
  ["Buk-M2 9S36 Fire Dome tr"]    = "SA17",
  -- SHORAD / self-contained launchers (spawn as single units = the detector)
  ["Osa 9A33 ln"]                 = "SA8",     -- SA-8 Gecko
  ["Strela-10M3"]                 = "SA13",    -- SA-13 Gopher
  ["Strela-10M3s"]                = "SA13",
  ["Tor 9A331"]                   = "SA15",    -- SA-15 Gauntlet
  ["HQ-17"]                       = "SA15CH",  -- HQ-17 (SA-15 export)
  ["2S6 Tunguska"]                = "SA19",    -- SA-19 Grison (PD)
  ["Roland ADS"]                  = "ROLAND",
  ["rapier_fsa_blindfire_radar"]  = "RAPIER",
  ["rapier_fsa_launcher"]         = "RAPIER",
  -- Point defense AAA / IR
  ["Gepard"]                      = "GEPARD",
  ["ZSU-23-4 Shilka"]             = "SHILKA",
  -- Newer-doctrine / mod systems
  ["SAMPT Arabel tr"]             = "SAMPT",
  ["Pantsir-S1"]                  = "SA22",
  ["2S6 Pantsir"]                 = "SA22",
}

-- EWR / standalone search radars: a group containing any of these (and no
-- SAM/PD type above) is classified as an EW radar.
AEGIS.EWR_TYPES = {
  ["1L13 EWR"]            = true,
  ["55G6 EWR"]            = true,
  ...
  ["Roland EWR"]          = true,
  ["Dog Ear radar"]       = true,
  ["RLS_19J6"]            = true,  -- 1L13 alt internal name
  ["S-300PS 64H6E sr"]    = true,  -- Big Bird SR acting as EW
}

--- Classify a single DCS unit type name → SAM system code, or nil.
--- Exact match first (fast, unambiguous), then case-insensitive substring.
function AEGIS._ClassifySamType(typeName)
  if not typeName then return nil end
  local exact = AEGIS.TYPE_TO_SYSTEM[typeName]
  if exact then return exact end
  local lt = typeName:lower()
  for t, code in pairs(AEGIS.TYPE_TO_SYSTEM) do
    if lt == t:lower() or lt:find(t:lower(), 1, true) then return code end
  end
  return nil
end

--- Is this DCS unit type an EWR / standalone search radar?
function AEGIS._IsEwrType(typeName)
  if not typeName then return false end
  if AEGIS.EWR_TYPES[typeName] then return true end
  local lt = typeName:lower()
  for t, _ in pairs(AEGIS.EWR_TYPES) do
    if lt == t:lower() or lt:find(t:lower(), 1, true) then return true end
  end
  return false
end
...
-- Dynamic discovery (v0.9.0-dynamic) — runtime spawn adoption
AEGIS.DYNAMIC_DISCOVERY         = true   -- master switch (default ON in this variant)
AEGIS.DYNAMIC_DEBOUNCE          = 3      -- s after first unit birth before classifying the group
AEGIS.DYNAMIC_STARTUP_GRACE     = 8      -- s after Activate() to ignore births (skip pre-placed ME units)
-- Dynamic networking (v0.9.1) — distance between a dynamic EW and a dynamic SAM
-- for them to join the same _DYN_<n> sector. Set 0 to disable networking and
-- fall back to v0.9.0 autonomous EMCON-only.
AEGIS.DYNAMIC_EW_RANGE_KM       = 80
...
function AEGIS:New(side, config)
  ...
  if self.dynamicDiscovery == nil then self.dynamicDiscovery = AEGIS.DYNAMIC_DISCOVERY end
  self.dynamicDebounce     = config.dynamicDiscoveryDebounce or AEGIS.DYNAMIC_DEBOUNCE
  self.dynamicStartupGrace = config.dynamicStartupGrace     or AEGIS.DYNAMIC_STARTUP_GRACE
  -- v0.9.1 networking: 0 = disable, fall back to v0.9.0 autonomous behaviour.
  self.dynamicEwRangeKm    = config.dynamicEwRangeKm        or AEGIS.DYNAMIC_EW_RANGE_KM
  -- Running counter for synthesised dynamic sectors ("_DYN_1", "_DYN_2", ...).
  self._dynSectorCounter   = 0
  -- Flag flipped by _RegisterDynamicEW / _RegisterDynamicSAM whenever a new
  -- sector enters play, so _PollNextSector rebuilds its order on the next
  -- rotation instead of every tick.
  self._dynSectorDirty     = false
  self.dynamicActiveSince  = nil   -- set when event handler registers (Activate)
  ...
  -- Dynamic discovery bookkeeping (v0.9.0-dynamic)
  self.dynamicGroups  = {}  -- groupName -> true (already adopted)
  self.dynamicPending = {}  -- groupName -> true (debounce timer in flight)
...
function AEGIS:_RegisterSAM(groupName, opts)
  if self.samSites[groupName] then return end
  local grp = Group.getByName(groupName)
  if not grp then self:_Log("SAM not found: " .. groupName, true); return end

  -- v0.9.0-dynamic: opts.sysType lets the dynamic discoverer supply the
  -- system code directly (runtime-spawned groups have non-AEGIS names that
  -- the SAM-TYPE- pattern below can't parse). When omitted, behaviour is
  -- identical to v0.8.4 (system code parsed from the group name).
  opts = opts or {}
  ...
  local sysType = opts.sysType or groupName:match("^SAM%-([%w]+)%-") or "UNKNOWN"
...
function AEGIS:_PollNextSector()
  ...
  -- v0.9.1: lazy rebuild of sectorPollOrder. _RegisterDynamicEW /
  -- _RegisterDynamicSAM flip _dynSectorDirty when they admit a new sector or
  -- promote a SAM out of _AUTO; we apply the change here so the new sector
  -- joins the round-robin on the next tick without restarting the timer (the
  -- doubled-poll concern from v0.9.0's "WHY AUTONOMOUS" note).
  if self._dynSectorDirty then
    self._dynSectorDirty = false
    self.sectorPollOrder = {}
    for name, sec in pairs(self.sectors) do
      if name ~= "_AUTO" and #sec.ew > 0 then
        table.insert(self.sectorPollOrder, name)
      end
    end
    table.sort(self.sectorPollOrder)
    -- Reset the round-robin index conservatively — pin to start so we don't
    -- skip a sector and don't double-tick the one we were about to read.
    self.sectorPollIndex = 0
    self.jammerPollCounter = 0
    if self.debug then
      self:_Log("v0.9.1 sectorPollOrder rebuilt (" .. #self.sectorPollOrder .. " sectors)")
    end
  end
...
function AEGIS:_RegisterEventHandler()
  ...
  self.eventHandler = {
    onEvent = function(_, event)
      ...
      elseif event.id == world.event.S_EVENT_BIRTH then
        ...
        -- v0.9.0-dynamic: adopt own-coalition air-defense groups spawned at runtime
        if aegis.dynamicDiscovery then
          local okD, errD = pcall(function() aegis:_OnBirthDynamic(event) end)
          if not okD then
            aegis:_Log("dynamic birth handler error: " .. tostring(errD), true)
          end
        end
  ...
  -- v0.9.0-dynamic: start the startup-grace clock so pre-placed ME ground
  -- units (which fire S_EVENT_BIRTH at mission start) are NOT auto-adopted.
  self.dynamicActiveSince = timer.getTime()
  self:_Log("Event handler registered (deaths + HARM detection"
            .. (self.eaEnabled and " + EA discovery" or "")
            .. (self.dynamicDiscovery and " + dynamic spawn discovery" or "") .. ")")
end

---------------------------------------------------------------------------
-- DYNAMIC DISCOVERY (v0.9.0-dynamic)
-- Adopt own-coalition air-defense groups that spawn AFTER mission start.
---------------------------------------------------------------------------

--- S_EVENT_BIRTH handler for runtime-spawned ground units. Filters to
--- own-coalition GROUND groups, debounces per group (a group's units all
--- birth together), then classifies + registers the group.
function AEGIS:_OnBirthDynamic(event)
  local u = event.initiator
  if not u or not u.getGroup then return end

  -- Ignore births during the startup grace window — pre-placed ME ground
  -- units fire S_EVENT_BIRTH at mission start; we don't want to auto-adopt them.
  if self.dynamicActiveSince
     and (timer.getTime() - self.dynamicActiveSince) < self.dynamicStartupGrace then
    return
  end

  -- Own-coalition only.
  local okCo, co = pcall(u.getCoalition, u)
  if not okCo or co ~= self.coalitionId then return end

  -- Must belong to a GROUND group.
  local grp = u:getGroup()
  if not grp or not grp.getCategory then return end
  local okCat, gcat = pcall(grp.getCategory, grp)
  if not okCat or gcat ~= Group.Category.GROUND then return end

  local gname = grp:getName()
  if not gname then return end

  -- Skip groups already known (static IADS) or already adopted/in-flight.
  if self.samSites[gname] or self.ewRadars[gname] or self.pdSites[gname] then return end
  if self.dynamicGroups[gname] or self.dynamicPending[gname] then return end

  -- Debounce: let the rest of the group finish spawning, then classify once.
  self.dynamicPending[gname] = true
  local aegis = self
  timer.scheduleFunction(function()
    aegis.dynamicPending[gname] = nil
    local ok, err = pcall(function() aegis:_RegisterDynamicGroup(gname) end)
    if not ok then
      aegis:_Log("dynamic register error (" .. gname .. "): " .. tostring(err), true)
    end
    return nil
  end, nil, timer.getTime() + self.dynamicDebounce)
end

--- Classify a spawned ground group by unit type and register it.
--- SAM/PD systems -> autonomous EMCON site. EWR -> tracked (not polled).
function AEGIS:_RegisterDynamicGroup(groupName)
  if self.dynamicGroups[groupName] then return end
  if self.samSites[groupName] or self.ewRadars[groupName] or self.pdSites[groupName] then return end
  ...
  -- 1) SAM/PD detection: first unit type that maps to a system code wins.
  local sysCode = nil
  for _, unit in ipairs(units) do
    sysCode = AEGIS._ClassifySamType(unit:getTypeName())
    if sysCode then break end
  end
  if sysCode then
    self.dynamicGroups[groupName] = true
    self:_RegisterDynamicSAM(groupName, sysCode)
    return
  end

  -- 2) EWR detection (only when no SAM/PD type is present in the group).
  for _, unit in ipairs(units) do
    if AEGIS._IsEwrType(unit:getTypeName()) then
      self.dynamicGroups[groupName] = true
      self:_RegisterDynamicEW(groupName)
      return
    end
  end
  -- Not an air-defense group — ignore (don't mark adopted).
end

--- v0.9.1 helper: return the centroid (x, z) of a group's live units, or nil
--- if the group has despawned. Cheap — no sqrt, just averaging.
function AEGIS:_GroupCenterPos(groupName)
  local grp = Group.getByName(groupName)
  ...
  for _, u in ipairs(units) do
    if u and u:isExist() then
      ...
      if ok and p then sx = sx + p.x; sz = sz + p.z; n = n + 1 end
    end
  end
...
--- v0.9.1 helper: find the nearest _DYN_<n> sector whose EW is within
--- `rangeKm` of `pos`. Returns the sector name or nil. Squared-distance
--- comparison (no sqrt) — matches the SAM WEZ check optimisation.
function AEGIS:_NearestDynSectorForPos(pos, rangeKm)
  if not pos or rangeKm <= 0 then return nil end
  local maxM2 = (rangeKm * 1000) ^ 2
  local bestSector, bestDist2 = nil, nil
  for name, sec in pairs(self.sectors) do
    if name:sub(1, 5) == "_DYN_" and #sec.ew > 0 then
      for _, ewName in ipairs(sec.ew) do
        local ew = self.ewRadars[ewName]
        if ew and ew.state ~= AEGIS.STATE.DESTROYED and ew.pos then
          local dx = ew.pos.x - pos.x
          local dz = ew.pos.z - pos.z
          local d2 = dx * dx + dz * dz
          if d2 <= maxM2 and (not bestDist2 or d2 < bestDist2) then
            bestSector = name
            bestDist2 = d2
          end
        end
...
--- v0.9.1 helper: move a SAM out of "_AUTO" into a fresh dynamic sector
--- created by a newly-arrived EW. Stops the SAM's autonomous EMCON loop —
--- the regular EW-fed poll takes over from here. Flips _dynSectorDirty so
--- _PollNextSector picks up the new layout on its next tick.
function AEGIS:_PromoteAutoSamToDynSector(samName, dynSector)
  local sam = self.samSites[samName]
  if not sam or sam.sector ~= "_AUTO" then return end
  local autoSec = self.sectors["_AUTO"]
  if autoSec then
    for i, n in ipairs(autoSec.sams) do
      if n == samName then table.remove(autoSec.sams, i); break end
    end
  end
  -- Cancel the autonomous timer chain; the new sector will drive EMCON via EW poll.
  self:_StopEMCON(samName)
  sam.sector = dynSector
  self:_EnsureSector(dynSector)
  table.insert(self.sectors[dynSector].sams, samName)
  self._dynSectorDirty = true
  self:_Log("*** DYN PROMOTE: " .. samName .. " -> sector " .. dynSector, true)
end

--- Register a runtime-spawned SAM/PD. v0.9.1: if a nearby dynamic EW sector
--- exists (within dynamicEwRangeKm), join it and let the EW poll drive
--- engagements. Otherwise fall back to v0.9.0 autonomous behaviour (sector
--- "_AUTO", self-driven EMCON cycle).
--- Reuses the v0.8.4 _RegisterSAM site model via opts.sysType.
--- PD-category systems are also adopted (a spawned short-range launcher with
--- no parent would otherwise never engage).
function AEGIS:_RegisterDynamicSAM(groupName, sysCode)
  self:_RegisterSAM(groupName, { sysType = sysCode })
  local sam = self.samSites[groupName]
  if not sam then
    self:_Log("dynamic SAM register failed: " .. groupName, true)
    return
  end

  -- v0.9.1: prefer an existing nearby dynamic EW sector.
  local sectorName = "_AUTO"
  if self.dynamicEwRangeKm > 0 and sam.pos then
    local dyn = self:_NearestDynSectorForPos(sam.pos, self.dynamicEwRangeKm)
    if dyn then sectorName = dyn end
  end
  sam.sector = sectorName
  self:_EnsureSector(sectorName)
  table.insert(self.sectors[sectorName].sams, groupName)

  -- Cold start (mirror Activate()): radar off, ALARM RED so there's no
  -- warm-up lag when EMCON peeks, weapons hold until a peek goes hot.
  local grp = Group.getByName(groupName)
  if grp and grp:isExist() then
    grp:enableEmission(false)
    local ctrl = grp:getController()
  ...
  if sectorName == "_AUTO" then
    self:_Log("*** DYNAMIC SAM adopted: " .. groupName .. " [" .. sysCode
              .. " " .. (sam.sysData.cat or "?") .. "] -> autonomous EMCON", true)
    -- v0.9.0 path: self-contained EMCON timer.
    self:_StartEMCON(groupName)
  else
    self._dynSectorDirty = true
    self:_Log("*** DYNAMIC SAM adopted: " .. groupName .. " [" .. sysCode
              .. " " .. (sam.sysData.cat or "?") .. "] -> networked sector " .. sectorName, true)
    -- EW-fed poll drives engagements; no autonomous EMCON timer here.
  end
end

--- Register a runtime-spawned EW radar. v0.9.1: creates a fresh "_DYN_<n>"
--- sector for the EW and promotes any "_AUTO" SAMs within dynamicEwRangeKm
--- into it. Flips _dynSectorDirty so _PollNextSector picks the new sector
--- up on its next rotation (no timer restart). When dynamicEwRangeKm == 0
--- the v0.9.0 behaviour returns: drop into the legacy "_DYN" tracking
--- sector and don't poll it.
function AEGIS:_RegisterDynamicEW(groupName)
  if self.dynamicEwRangeKm <= 0 then
    self:_RegisterEW(groupName, "_DYN")
    self:_Log("*** DYNAMIC EW adopted: " .. groupName
              .. " -> tracked (networking disabled)", true)
    return
  end

  self._dynSectorCounter = self._dynSectorCounter + 1
  local dynSector = "_DYN_" .. self._dynSectorCounter
  self:_EnsureSector(dynSector)
  self:_RegisterEW(groupName, dynSector)
  self._dynSectorDirty = true

  -- Promote any nearby autonomous SAMs into this sector.
  local ewPos = self:_GroupCenterPos(groupName)
  local promoted = 0
  if ewPos then
    local maxM2 = (self.dynamicEwRangeKm * 1000) ^ 2
    local autoSec = self.sectors["_AUTO"]
    if autoSec then
      -- Snapshot the SAM list — _PromoteAutoSamToDynSector mutates it.
      local snap = {}
      for _, samName in ipairs(autoSec.sams) do snap[#snap + 1] = samName end
      for _, samName in ipairs(snap) do
        local sam = self.samSites[samName]
        if sam and sam.pos then
          local dx = sam.pos.x - ewPos.x
          local dz = sam.pos.z - ewPos.z
          if dx * dx + dz * dz <= maxM2 then
            self:_PromoteAutoSamToDynSector(samName, dynSector)
            promoted = promoted + 1
          end
        end
      end
  ...
  self:_Log("*** DYNAMIC EW adopted: " .. groupName
            .. " -> sector " .. dynSector
            .. " (promoted " .. promoted .. " _AUTO SAM"
            .. (promoted == 1 and "" or "s") .. " into the network)", true)
end
