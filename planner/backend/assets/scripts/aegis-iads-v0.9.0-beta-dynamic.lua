...
--[[
================================================================================
  AEGIS IADS v0.9.0-beta-dynamic -- Event-Driven IADS w/ runtime spawn discovery

  *** DYNAMIC VARIANT — DERIVED FROM aegis-iads-v0.8.4-beta.lua ***
  ...
  *** the vetted/stable build. This file only ADDS dynamic discovery.      ***

  WHAT'S DIFFERENT FROM v0.8.4
  ----------------------------
  ...
  This variant adds TYPE-BASED dynamic discovery:
    ...
    * Each new group is classified by UNIT TYPE (not name) against
      ...
      AEGIS.EWR_TYPES.
    * A detected SAM/PD group is registered as an AUTONOMOUS site (sector
      "_AUTO") and immediately begins EMCON cycling via the existing
      _StartEMCON() timer chain — it self-defends and pops its radar on
      realistically when threats enter range. No EW/C2 wiring required.
    * Detected EW groups are registered + tracked (death/visualizer) but are
      NOT live-polled in v0.9.0 (autonomous SAMs don't consume an EW feed).
      Networked dynamic sectors (EW + C2 fed) are a v0.9.1 follow-up.

  WHY AUTONOMOUS (and not full network integration):
    The poll loop (_StartEWPoll) is a single recurring scheduler — restarting
    it to admit a new sector would double-poll. And a networked SAM only behaves
    differently from EMCON cycling when its sector has BOTH an EW AND a CMD
    center; runtime spawns never include a CMD post, so a "networked" dynamic
    SAM collapses to EMCON anyway. Autonomous registration is therefore both
    SAFER (no mutation of the running poll) and functionally equivalent here.
...
--- Register a runtime-spawned SAM/PD as an AUTONOMOUS site and start its
--- EMCON cycle. Reuses the v0.8.4 _RegisterSAM site model via opts.sysType.
--- PD-category systems are also adopted as autonomous SAM sites (a spawned
--- short-range launcher with no parent would otherwise never engage).
function AEGIS:_RegisterDynamicSAM(groupName, sysCode)
  ...
  -- Autonomous sector (no EW/C2 dependency).
  sam.sector = "_AUTO"
  self:_EnsureSector("_AUTO")
  ...
  -- Begin autonomous EMCON cycling (self-contained timer chain).
  self:_StartEMCON(groupName)
end

--- Register a runtime-spawned EW radar. Tracked for death/visualizer; NOT
--- live-polled in v0.9.0 (autonomous SAMs don't consume an EW feed). Live
--- networking of dynamic EW sectors is a v0.9.1 follow-up.
function AEGIS:_RegisterDynamicEW(groupName)
  self:_RegisterEW(groupName, "_DYN")
  self:_Log("*** DYNAMIC EW adopted: " .. groupName
            .. " -> tracked (autonomous, not networked in v0.9.0)", true)
end
