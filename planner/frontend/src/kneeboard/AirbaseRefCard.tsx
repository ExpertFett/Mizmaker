        ...
        <tbody>
          {sorted.map((ab, i) => (
            <tr key={ab.name + i} style={{ background: i % 2 === 0 ? 'transparent' : ROW_ALT }}>
              ...
              <td style={{ ...cell, textAlign: 'center', fontSize: 9, color: DIM }}>{fmtCoord(ab.lat, ab.lon)}</td>
              <td style={{ ...cell, textAlign: 'center', fontSize: 9, color: DIM }}>{fmtLatLon(ab.lat, ab.lon)}</td>
              <td style={{
                ...cell,
                textAlign: 'center',
                fontSize: 9,
