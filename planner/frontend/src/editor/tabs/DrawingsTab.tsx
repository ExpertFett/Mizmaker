...
export function DrawingsTab() {
  ...
  return (
    <div style={{ maxWidth: 700 }}>
      ...
      <div style={{ ...card, background: '#0a1520', fontSize: 12, color: '#5a7a8a', lineHeight: 1.6 }}>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
        <div style={{ ...card, flex: 1, textAlign: 'center', marginBottom: 0 }}>
          ...
          <div style={{ fontSize: 11, color: '#5a7a8a' }}>Racetracks</div>
        </div>
        <div style={{ ...card, flex: 1, textAlign: 'center', marginBottom: 0 }}>
          <div style={{ fontSize: 22, fontWeight: 700, color: '#4a8fd4' }}>{corridors.length}</div>
          <div style={{ fontSize: 11, color: '#5a7a8a' }}>Corridors</div>
        </div>
        <div style={{ ...card, flex: 1, textAlign: 'center', marginBottom: 0 }}>
          <div style={{ fontSize: 22, fontWeight: 700, color: '#ccdae8' }}>{drawings.filter((d) => d.visible).length}</div>
          <div style={{ fontSize: 11, color: '#5a7a8a' }}>Visible</div>
        </div>
      </div>

      {/* Drawing list */}
      {drawings.length === 0 && (
        <div style={{ ...card, color: '#5a7a8a', textAlign: 'center', fontSize: 13 }}>
      ...
      {drawings.map((d) => {
        ...
        return (
          <div key={d.id} style={{
            ...card,
            border: isSelected ? `1px solid ${d.color}` : '1px solid #1a2a3a',
            cursor: 'pointer',
                    ...
                    <div style={{ display: 'flex', gap: 6 }}>
                      {(['dashed', 'solid'] as const).map((ls) => (
                        <button key={ls} onClick={() => updateDrawing(d.id, { lineStyle: ls })} style={{
                          ...btn,
                          background: d.lineStyle === ls ? '#1a3a5a' : '#0f1a28',
                          border: `1px solid ${d.lineStyle === ls ? '#4a8fd4' : '#1a2a3a'}`,
                          color: d.lineStyle === ls ? '#ccdae8' : '#5a7a8a',
