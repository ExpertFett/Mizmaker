...
function FlightCard({ group, color, isSelected, onSelect }: {
  ...
  return (
    <div
      style={{
        background: isSelected ? 'rgba(255,255,255,0.05)' : '#8c9ba2',
        border: `1px solid ${isSelected ? color : '#4a5258'}`,
        borderRadius: 5,
        overflow: 'hidden',
    ...
    >
      ...
      >
        <div>
          <span style={{ fontWeight: 600, color: '#1a1f25', fontSize: 14 }}>{group.groupName}</span>
          <span style={{ color: '#3a4248', fontSize: 12, marginLeft: 8 }}>{airframe}</span>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          <span style={{ color: '#3a4248', fontSize: 12 }}>{clientCount} slots</span>
          <button
            onClick={handleEye}
            title={hidden ? 'Show route' : 'Hide route'}
            style={{
              background: 'transparent', border: 'none', cursor: 'pointer',
              fontSize: 15, color: hidden ? '#4a5258' : '#3a4248', padding: '0 2px', lineHeight: 1,
            }}
          >
            {hidden ? '\u25CB' : '\u25C9'}
          </button>
      ...
      <div style={{ padding: '0 10px 6px 13px', fontSize: 12, color: '#3a4248', display: 'flex', gap: 8 }}>
      ...
      {expanded && (
        <div style={{ borderTop: '1px solid #4a5258', maxHeight: 200, overflow: 'auto' }}>
...
function CompactWpRow({ wp }: { wp: Waypoint }) {
  ...
  return (
    <tr style={{ borderBottom: '1px solid #8c9ba2', opacity: isWp0 ? 0.4 : 1 }}>
      <td style={{ padding: '3px 6px', fontFamily: "'B612 Mono', monospace", color: '#3a4248', width: 24 }}>{wp.waypoint_number}</td>
      ...
      <td style={{ padding: '3px 6px', textAlign: 'right', fontFamily: "'B612 Mono', monospace", color: '#3a4248' }}>{spdKts || '-'}</td>
      <td style={{ padding: '3px 6px', textAlign: 'right', fontFamily: "'B612 Mono', monospace", color: '#3a4248' }}>{distNm}</td>
      <td style={{ padding: '3px 6px', textAlign: 'right', fontFamily: "'B612 Mono', monospace", color: '#3a4248' }}>{brg}</td>
    </tr>
  );
}
